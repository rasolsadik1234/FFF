...
{id:23,page:'netflix',name:'Netflix Premium',price:250000,img:'https://69f2840d1c78705e2c4b5c7d.imgix.net/netflix_bm_w900_q80.jpg?w=900&h=900',desc:'اشتراك Premium.',duration:'سنة',details:'يتم الاتفاق داخل التكت.'},
...
let cart=JSON.parse(localStorage.getItem('bm_cart_exact'))||[]
const $=id=>document.getElementById(id)
function money(n){return 'IQD '+n.toLocaleString('en-US')}
function pageName(id){return (pages.find(p=>p.id===id)||{}).name||id}
function save(){localStorage.setItem('bm_cart_exact',JSON.stringify(cart))}
...
function renderProducts(page){const inp=document.querySelector(`.pageSearch[data-page="${page}"]`);const q=inp?inp.value.toLowerCase():'';document.querySelector(`[data-grid="${page}"]`).innerHTML=products.filter(p=>p.page===page&&(p.name.toLowerCase().includes(q)||p.desc.toLowerCase().includes(q)||p.duration.toLowerCase().includes(q))).map(productCard).join('')}
